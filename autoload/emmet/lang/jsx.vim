...
function! emmet#lang#jsx#findTokens(str) abort
  let str = a:str
...
function! emmet#lang#jsx#parseIntoTree(abbr, type) abort
  let abbr = a:abbr
  let type = a:type
  ...
  if len(type) == 0 | let type = 'jsx' | endif
  ...
  while len(abbr)
    ...
    if !empty(snippets)
      ...
      if has_key(snippets, snippet_name)
        ...
        if type == 'jsx' && snippet !~ '^\s*[{\[<]'
           return emmet#lang#jsx#parseIntoTree(snippet, a:type)
        endif
    ...
    if !empty(default_attributes)
      for pat in [current.name, tag_name]
        if has_key(default_attributes, pat)
          ...
          if has_key(settings.jsx.default_attributes, current.name)
...
function! emmet#lang#jsx#toString(settings, current, type, inline, filters, itemno, indent) abort
  ...
  let empty_element_suffix = emmet#getResource(type, 'empty_element_suffix', settings.jsx.empty_element_suffix)
  ...
  elseif stridx(','.empty_elements.',', ','.current_name.',') != -1
    ...
    if nc > 0
      for n in range(nc)
        ...
        if child.multiplier > 1 || (child.multiplier == 1 && len(child.child) > 0 && stridx(','.settings.jsx.inline_elements.',', ','.current_name.',') == -1) || settings.jsx.block_all_childless
          let str .= "\n" . indent
          let dr = 1
        elseif len(current_name) > 0 && stridx(','.settings.jsx.inline_elements.',', ','.current_name.',') == -1
          if nc > 1 || (len(child.name) > 0 && stridx(','.settings.jsx.inline_elements.',', ','.child.name.',') == -1)
            let str .= "\n" . indent
            let dr = 1
  ...
  if len(current_name) > 0 && current.multiplier > 0 || stridx(','.settings.jsx.block_elements.',', ','.current_name.',') != -1
    let str .= "\n"
  endif
  return str
endfunction

function! emmet#lang#jsx#imageSize() abort
  ...
  let current = emmet#lang#jsx#parseTag(content)
  ...
  let jsx = substitute(emmet#toString(current, 'jsx', 1), '\n', '', '')
  let jsx = substitute(jsx, '\${cursor}', '', '')
  call emmet#util#setContent(img_region, jsx)
endfunction

function! emmet#lang#jsx#imageEncode() abort
  let img_region = emmet#util#searchRegion('<img\s', '>')
  if !emmet#util#regionIsValid(img_region) || !emmet#util#cursorInRegion(img_region)
    return
  endif
  ...
  let content = substitute(emmet#toString(current, 'jsx', 1), '\n', '', '')
  let content = substitute(content, '\${cursor}', '', '')
  call emmet#util#setContent(img_region, content)
endfunction

function! emmet#lang#jsx#parseTag(tag) abort
  let current = emmet#newNode()
...
function! emmet#lang#jsx#toggleComment() abort
  let orgpos = getpos('.')
  let curpos = getpos('.')
  let mx = '<\%#[^>]*>'
  while 1
...
function! emmet#lang#jsx#balanceTag(flag) range abort
  ...
  if a:flag > 0
    ...
    while 1
      ...
      if stridx(','.settings.jsx.empty_elements.',', ','.tag_name.',') != -1
...
function! emmet#lang#jsx#moveNextPrevItem(flag) abort
...
function! emmet#lang#jsx#moveNextPrev(flag) abort
...
function! emmet#lang#jsx#splitJoinTag() abort
  let curpos = emmet#util#getcurpos()
...
function! emmet#lang#jsx#removeTag() abort
  let curpos = emmet#util#getcurpos()
  let mx = '<\(/\{0,1}[a-zA-Z][-a-zA-Z0-9:_\-]*\)\%(\%(\s[a-zA-Z][a-zA-Z0-9]\+=\%([^"'' \t]\+\|"[^"]\{-}"\|''[^'']\{-}''\)\s*\)*\)\s*\%(/\{0,1}\)>'
...
function! emmet#lang#jsx#mergeLines() abort
  let curpos = emmet#util#getcurpos()
  let settings = emmet#getSettings()
